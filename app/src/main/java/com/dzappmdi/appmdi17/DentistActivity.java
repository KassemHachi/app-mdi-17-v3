...
public class DentistActivity extends AppCompatActivity {
		...
		listview1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				if (_position == 0) {
					...
					info.setMessage("حي بوتريفيس مقابل موقف نقل الحضري\n( طريق مغسلة السيارات وراء مسجد سيدي نايل) ");
					info.setNeutralButton("اتصل", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
							call.setAction(Intent.ACTION_DIAL);
							call.setData(Uri.parse("tel:0669796812"));
							startActivity(call);
						}
					});
					info.setPositiveButton("نسخ المعلومات", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
							((ClipboardManager) getSystemService(getApplicationContext().CLIPBOARD_SERVICE)).setPrimaryClip(ClipData.newPlainText("clipboard", mdi.get((int)(_position)).concat("حي بوتريفيس مقابل موقف نقل الحضري\n( طريق مغسلة السيارات وراء مسجد سيدي نايل)\ntel:0669796812")));
				...
				if (_position == 1) {
					info.setTitle(mdi.get((int)(_position)));
					info.setMessage("دار البارود بجنب صيدلية عياطة");
					info.setPositiveButton("نسخ المعلومات", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
							((ClipboardManager) getSystemService(getApplicationContext().CLIPBOARD_SERVICE)).setPrimaryClip(ClipData.newPlainText("clipboard", mdi.get((int)(_position)).concat("دار البارود بجنب صيدلية عياطة")));
							SketchwareUtil.showMessage(getApplicationContext(), "تم النسخ! ");
						}
					});
					info.create().show();
	...
	private void initializeLogic() {
		mdi.add("محمدي عبد القادر ");
		mdi.add("برق سليمان كمال");
