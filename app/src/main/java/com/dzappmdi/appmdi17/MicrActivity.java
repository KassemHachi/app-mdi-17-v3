...
public class MicrActivity extends AppCompatActivity {
		...
		listview1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				if (_position == 0) {
					...
					info.setMessage("حي عين الشيح بجوار بلدية عين الشيح ");
					info.setPositiveButton("نسخ المعلومات", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
							((ClipboardManager) getSystemService(getApplicationContext().CLIPBOARD_SERVICE)).setPrimaryClip(ClipData.newPlainText("clipboard", mdi.get((int)(_position)).concat("حي عين الشيح بجوار بلدية عين الشيح ")));
				...
				if (_position == 1) {
					info.setTitle(mdi.get((int)(_position)));
					info.setMessage("حي باب الشارف مقابل حمام الحرفة ");
					info.setPositiveButton("نسخ المعلومات", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
	...
	private void initializeLogic() {
		mdi.add("مخبر التحليل خليل");
		mdi.add("مخبر التحاليل باب الشارف");
