...
public class OphthalmologyActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.ophthalmology);
		initialize();
		initializeLogic();
	}
	...
	private void initialize() {
		...
		listview1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				if (_position == 0) {
					...
					info.setNeutralButton("اتصل", new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface _dialog, int _which) {
							...
							call.setData(Uri.parse("tel:0777701954"));
							startActivity(call);
						}
					});
				...
				if (_position == 1) {
					info.setTitle(mdi.get((int)(_position)));
					info.setMessage("حي 5 جويلية أمام المسجد ");
					info.setNeutralButton("اتصل", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
							call.setAction(Intent.ACTION_DIAL);
							call.setData(Uri.parse("tel:027901504"));
							startActivity(call);
						}
					});
					info.setPositiveButton("نسخ المعلومات", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
							((ClipboardManager) getSystemService(getApplicationContext().CLIPBOARD_SERVICE)).setPrimaryClip(ClipData.newPlainText("clipboard", mdi.get((int)(_position)).concat("حي 5 جويلية أمام المسجد\ntel:027901504")));
							SketchwareUtil.showMessage(getApplicationContext(), "تم النسخ! ");
						}
					});
					info.create().show();
				...
				if (_position == 2) {
					info.setTitle(mdi.get((int)(_position)));
					info.setMessage("حي المسجد الجديد بناية 243 رقم 5");
					info.setNeutralButton("اتصل", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
							call.setAction(Intent.ACTION_DIAL);
							call.setData(Uri.parse("tel:027873495"));
							startActivity(call);
						}
					});
					info.setPositiveButton("نسخ المعلومات", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
							((ClipboardManager) getSystemService(getApplicationContext().CLIPBOARD_SERVICE)).setPrimaryClip(ClipData.newPlainText("clipboard", mdi.get((int)(_position)).concat("حي المسجد الجديد بناية 243 رقم 5\ntel:027873495")));
							SketchwareUtil.showMessage(getApplicationContext(), "تم النسخ! ");
						}
					});
					info.create().show();
